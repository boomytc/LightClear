...
def build_metrics_rows(
    ...
    return [
        {
            "指标类别": "音频时长 (秒)",
            "混合音频": f"{metrics['duration']:.2f}",
            "分离音频": f"{metrics['duration']:.2f}",
            "变化量": "无变化",
            "变化说明": "时长保持不变",
        },
        {
            "指标类别": "采样率 (Hz)",
            "混合音频": str(sample_rate),
            "分离音频": str(sample_rate),
            "变化量": "无变化",
            "变化说明": "采样率保持不变",
        },
        {
            "指标类别": "RMS 能量",
            "混合音频": f"{original_rms:.4f}",
            "分离音频": f"{enhanced_rms:.4f}",
            "变化量": f"{energy_change:+.2f}%",
            "变化说明": energy_desc,
        },
        {
            "指标类别": "峰值",
            "混合音频": f"{original_peak:.4f}",
            "分离音频": f"{enhanced_peak:.4f}",
            "变化量": f"{peak_change:+.2f}%",
            "变化说明": peak_desc,
        },
        {
            "指标类别": "动态范围 (dB)",
            "混合音频": f"{original_dr:.2f}",
            "分离音频": f"{enhanced_dr:.2f}",
        ...
        },
        {
            "指标类别": "差异能量比 (dB)",
            "混合音频": "-",
            "分离音频": f"{snr_improvement:.2f}",
            "变化量": f"{snr_improvement:+.2f} dB",
            "变化说明": snr_desc,
        },
        {
            "指标类别": "谱质心 (Hz)",
            "混合音频": f"{original_centroid:.2f}",
            "分离音频": f"{enhanced_centroid:.2f}",
            "变化量": f"{centroid_change:+.2f} Hz",
            "变化说明": centroid_desc,
        },
        {
            "指标类别": "零交叉率",
            "混合音频": f"{original_zcr:.4f}",
            "分离音频": f"{enhanced_zcr:.4f}",
            "变化量": f"{zcr_change:+.2f}%",
            "变化说明": zcr_desc,
        },
    ]
...
def make_waveform_figure(
    ...
    *,
    ...
    ax1.set_title("混合音频波形", fontsize=12, fontweight="bold")
    ax1.set_ylabel("振幅")
    ax1.grid(True, alpha=0.25)

    ax2.plot(time_axis, enhanced_audio[:max_samples], color="#0f766e", alpha=0.85, linewidth=0.6)
    ax2.set_title("分离音频波形", fontsize=12, fontweight="bold")
    ax2.set_xlabel("时间 (秒)")
    ax2.set_ylabel("振幅")
...
def make_power_spectrum_figure(
    ...
    ax.semilogx(f1, 10 * np.log10(psd1 + 1e-10), color="#2563eb", alpha=0.8, label="混合音频")
    ax.semilogx(f2, 10 * np.log10(psd2 + 1e-10), color="#0f766e", alpha=0.85, label="分离音频")
...
def make_mel_spectrum_figure(
    ...
    ax1.set_title("混合音频 Mel 频谱图", fontsize=12, fontweight="bold")
    ...
    ax2.set_title("分离音频 Mel 频谱图", fontsize=12, fontweight="bold")
    ax2.set_xlabel("时间 (秒)")
