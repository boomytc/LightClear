...
SAMPLE_AUDIO_FILES = ("assets/input_ss.wav",)
NUM_SPEAKERS = 2
...
def load_mossformer2_ss(product_root: Path) -> ModelHandle:
...
def make_output_path(output_dir: Path, input_path: Path) -> Path:
    ...
    return output_dir / f"{path.stem}_separated_{timestamp}{path.suffix}"


def expected_separation_paths(base_output_path: Path) -> list[Path]:
    return [
        base_output_path.with_name(f"{base_output_path.stem}_s{speaker}{base_output_path.suffix}")
