...
APP_OUTPUT_ROOT = PROJECT_ROOT / "outputs" / "speech_super_resolution_web"
...
app = FastAPI(title="LightClear Speech Super Resolution Web", version="1.0.0")
...
def get_model_handle() -> ModelHandle:
    ...
    with _model_lock:
        if _model_handle is None:
            _model_handle = load_mossformer2_sr(PROJECT_ROOT)
...
def store_job(result: SuperResolutionResult, analysis: dict[str, object]) -> JobRecord:
...
def build_result_payload(record: JobRecord) -> dict[str, object]:
    ...
    return {
        ...
        "output_audio_url": f"/api/jobs/{record.job_id}/audio/super-resolved",
        ...
        "logs": [
            ...
            f"超分输出: {project_relative(result.output_path)}",
...
def health() -> dict[str, object]:
    ...
    return {
        "app": "speech_super_resolution_web",
...
async def super_resolve(
    ...
    try:
        ...
        with _inference_lock:
            result = super_resolve_audio_file(
