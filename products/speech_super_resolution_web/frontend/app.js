...
function setBusy(isBusy) {
  state.busy = isBusy;
  el.enhanceButtonText.textContent = isBusy ? "处理中" : "开始超分";
...
function renderResult(data) {
  el.enhancedAudio.src = data.output_audio_url;
  el.outputName.textContent = data.output_name;
  el.outputName.classList.remove("muted");
  el.outputPath.textContent = data.output_path;
  el.downloadLink.href = data.download_url;
  el.downloadLink.download = data.output_name;
...
async function runEnhancement() {
  ...
  try {
    const data = await apiJson("/api/super-resolve", {
      method: "POST",
      body: formData,
    });
    renderResult(data);
