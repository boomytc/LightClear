...
客户端默认串联顺序：分离 → 增强 → 超分（未勾选的跳过）。勾选分离后再勾增强/超分，会对每一路说话人各跑一次。切换样例时预勾该样例的 `suggested_tools`，禁止默认三件全开。磁盘任务可列出并打开；选中某步会拉该步分析。三件工具的 `available` 都要求 `last_best_checkpoint` 列出的权重文件齐全。
