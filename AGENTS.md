...
- 将本目录视为独立项目根目录，用于环境、安装、运行、demo、WebUI、模型索引和输出文件。
...
- 模型权重默认使用中心目录：`/Users/boom/Model/SE`、`/Users/boom/Model/SS`、`/Users/boom/Model/SR`。
- 除上述模型中心目录外，不要在脚本里硬编码 `/Users/boom/...` 这类机器绝对路径。
- demo 和 WebUI 入口需要从项目根目录启动，确保 `assets/`、`outputs/`、`third_party/` 的相对路径一致。
...
- `third_party/clearvoice/config/inference/`：各模型的推理配置；`checkpoint_dir` 与 `/Users/boom/Model/{SE,SS,SR}/` 下中心模型目录约定必须同步。
...
- `models/*/models.txt`：模型下载和中心目录索引，不存放真实权重，不等于本地权重一定已经完整存在。
...
- 如果新增或改名模型，至少同步检查：
  ...
  - `/Users/boom/Model/{SE,SS,SR}/` 下的中心模型目录
...
- 不要在 demo 中增加 Hugging Face / ModelScope 下载流程、训练流程或微调流程，除非用户明确要求；默认只针对已放入 `/Users/boom/Model/{SE,SS,SR}/` 的本地模型做加载推理。
...
- 运行完整推理前先确认对应模型权重目录存在于 `/Users/boom/Model/{SE,SS,SR}/`，并与 `third_party/clearvoice/config/inference/*.yaml` 中的 `checkpoint_dir` 一致。
...
- 清理时保留 `demo/`、`assets/`、`models/*/models.txt`、`third_party/clearvoice/config/` 和 `products/*_web/` 的公开入口；只清理明确的缓存、输出、项目内残留模型权重和临时文件。
