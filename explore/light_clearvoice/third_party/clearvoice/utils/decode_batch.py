...
def decode_one_audio_batch(model, device, inputs, args):
...
def decode_one_audio_mossformer2_ss_16k(model, device, inputs, args):
    ...
    input_len = inputs.shape[-1]  # Get the length of the input audio
    ...
    if decode_do_segment:
        outputs = np.zeros((args.num_spks, b, t))  # Initialize output array for each speaker
        ...
        while current_idx + window <= t:
            ...
            for spk in range(args.num_spks):
                ...
                tmp_out_list[spk] = tmp_out_list[spk].detach().cpu().numpy()
                if current_idx == 0:
        ...
        for spk in range(args.num_spks):
            out.append(outputs[spk, :, :input_len])  # Append outputs for each speaker
    else:
        # If no segmentation is required, process the entire input
        out_list = model(inputs)
        for spk in range(args.num_spks):
            out.append(out_list[spk][:, :input_len].detach().cpu().numpy())  # Append output for each speaker
    ...
    return np.array(out)  # Return the list of normalized outputs
...
def decode_one_audio_frcrn_se_16k(model, device, inputs, args):
    ...
    if decode_do_segment:
        ...
        # If no segmentation is required, process the entire input
        outputs = model.inference_batch(inputs).detach().cpu().numpy()  # Inference on full input

    return outputs[:, :input_len]  # Return the decoded audio output
...
def decode_one_audio_mossformer2_se_48k(model, device, inputs, args):
    ...
    if input_len > args.sampling_rate * args.one_time_decode_length:  # 20 seconds
        ...
        if online_decoding:
            ...
            b, t = inputs.shape  # Update length after potential padding
            ...
            while current_idx + window <= t:
                ...
                for batch_idx in range(b):
                    ...
                    fbanks = compute_fbank(audio_segment[batch_idx:batch_idx+1,:], args)
        ...
        for batch_idx in range(b):
            fbanks = compute_fbank(audio[batch_idx:batch_idx+1,:], args)
            # Compute deltas for filter banks
...
def decode_one_audio_mossformer2_sr_48k(model, device, inputs, args):
    ...
    """
    b, input_len = inputs.shape  # Get the length of the input audio
    ...
    if input_len > args.sampling_rate * args.one_time_decode_length:  # 20 seconds
        ...
        if online_decoding:
            window = int(args.sampling_rate * args.decode_window)  # Define window length (e.g., 4s for 48kHz)
            stride = int(window * 0.75)  # Define stride length (e.g., 3s for 48kHz)
            t = inputs.shape[1]  # Update length after potential padding
            ...
            audio = torch.from_numpy(inputs).type(torch.FloatTensor)  # Convert to Torch tensor
            t = audio.shape[1]  # Update length after conversion
            outputs = torch.from_numpy(np.zeros((b,t)))  # Initialize output tensor
            give_up_length = (window - stride) // 2  # Determine length to ignore at the edges
            dfsmn_memory_length = 0  # Placeholder for potential memory length
            current_idx = 0  # Initialize current index for sliding window
