...
- 本模块是 Demucs 这一份上游运行时的探索家族，覆盖人声隔离、四轨分轨、同四轨质量档和六轨分轨。不要折进 `light_clearvoice`，也不要按声部再拆模块。
...
- `third_party/demucs` 是上游 `adefossez/demucs` v4.1.0（`eeac1d15891af95b1288d2884b95baa3e5baa96c`）的 vendor 副本。允许在这份副本上做本模块需要的上游修改。官方仓库更新后单独判断是否值得同步，不要自动覆盖本地改动。产品 `vocal_isolate_web` 从本模块 `third_party/demucs` 再拷一份，不要从仓库外的 Demucs clone 直拷。
...
- `htdemucs_ft` 是同一四轨合同的质量档（四专家 bag，更慢）。`htdemucs_6s` 是六轨（+guitar / piano），不要当成更好的人声隔离。
...
- 完整推理前确认对应 `models/<model>/` 存在。缺权重是环境限制，不是模块缺陷。

## Cleanup

- 可清理 `__pycache__`、`.pytest_cache`、`outputs/`、一次性验证产物。
- 不要清理 `demo/`、`docs/`、`third_party/` 或 `models/README.md`。`models/<model>/` 下的权重可按需删掉后重装。
