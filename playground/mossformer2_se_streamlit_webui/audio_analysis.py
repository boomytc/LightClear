...
def make_waveform_figure(
    ...
    *,
    ...
    if show_original and show_enhanced:
        ...
        if show_original:
            ax.plot(time_axis, original_audio[:max_samples], color="#2563eb", alpha=0.8, linewidth=0.6, label="原始音频")
        if show_enhanced:
            ax.plot(time_axis, enhanced_audio[:max_samples], color="#0f766e", alpha=0.85, linewidth=0.6, label="增强音频")
...
def make_power_spectrum_figure(
    ...
    ax.semilogx(f2, 10 * np.log10(psd2 + 1e-10), color="#0f766e", alpha=0.85, label="增强音频")
...
def make_mel_spectrum_figure(
    ...
    ax2.set_title("增强音频 Mel 频谱图", fontsize=12, fontweight="bold")
    ax2.set_xlabel("时间 (秒)")
