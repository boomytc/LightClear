# MossFormer2 SE FastAPI WebUI

前后端分离的 MossFormer2_SE_48K 语音增强 playground。后端使用 FastAPI 暴露 `/api/*`，前端是 `frontend/` 下的静态 HTML/CSS/JS/Tailwind 页面。
...
默认输出写入 `outputs/mossformer2_se_web/enhanced/`，上传缓存写入 `outputs/mossformer2_se_web/uploads/`。
